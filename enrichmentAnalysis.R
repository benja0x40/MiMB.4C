...
experiment.design <- read.delim("Experiment_Design/GSE23887_dm6.txt", stringsAsFactors=F)

# Read raw data
array.data.format <- file.formats$nimblegen.pair
r1.ct <- readData(experiment.design$SAMPLE_PATH[1], array.data.format)
r1.4C <- readData(experiment.design$SAMPLE_PATH[2], array.data.format)
r2.ct <- readData(experiment.design$SAMPLE_PATH[3], array.data.format)
r2.4C <- readData(experiment.design$SAMPLE_PATH[4], array.data.format)

# =============================================================================.
# Read updated array design
...
# Match array data to updated design
...
# Match probes to fragments using the genomic intervals
...
res <- normalizeArrayData( # Replicate 1
  r1.A, r1.M, name="r1_norm", plots=TRUE
)
r1.A <- res$A; r1.M <- res$M
res <- normalizeArrayData( # Replicate 2
  r2.A, r2.M, name="r2_norm", plots=TRUE
)
r2.A <- res$A; r2.M <- res$M
...
# Reject also probes associated to a poor quality signal in both replicates
...
# =============================================================================.
# Identify chromosome interaction domains
sgm <- segmentation(
...
# Visualize max. resolution and max. scale chromosome interaction domains
...
# Retreive genomic coordinates of chromosome interaction domains
